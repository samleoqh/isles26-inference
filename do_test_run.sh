...
CONTAINER_NAME="isles26-ensemble_container"
...
INVOKE_TIMEOUT_SECONDS=0  # 0 = curl --max-time 0 = NO timeout (the local harness must not cap inference; the platform's own per-case limit applies on GC)
